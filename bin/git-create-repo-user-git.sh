...
usage() {
	...
	echo "  $CMD "
        # echo "  ssh -t -p $PORT jdg@$HOST 'REPO=\"RepoName\" ; /usr/local/syssetup/bin/git-create-repo-user-git.sh repo-name '"
        echo
        #exit 1
}
...
if [[ ! -d "$DIR" ]]; then
	...
	# echo "[running]> $CMD "
	# # $CMD
	# echo "/usr/local/syssetup/bin/git-create-repo-user-git.sh $REPNAME" | ssh -t -p $PORT jdg@$HOST '/bin/bash -l -s'
	# exit
fi
