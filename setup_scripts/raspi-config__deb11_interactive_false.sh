#!/bin/sh
#= raspi-config.local-copy-edit.sh

# jdg: this is a local-copy and edit of original file: 
...
is_kms() {
  if grep -s -q okay /proc/device-tree/soc/v3d@*/status \
                     /proc/device-tree/axi/v3d@*/status \
...
do_boot_order() {
  ...
  if [ $? -eq 0 ]; then
    ...
    FILNAME="none"
    if grep FIRMWARE_RELEASE_STATUS /etc/default/rpi-eeprom-update | grep -q "latest"; then
      EEPATH="/lib/firmware/raspberrypi/bootloader/latest/pieeprom*.bin"
    else
      EEPATH="/lib/firmware/raspberrypi/bootloader/default/pieeprom*.bin"
    fi
...
do_boot_rom() {
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ "$DEFAULTS" -eq 0 ]; then # yes
      if grep FIRMWARE_RELEASE_STATUS /etc/default/rpi-eeprom-update | grep -q "latest"; then
        EEPATH="/lib/firmware/raspberrypi/bootloader/latest/"
      else
        EEPATH="/lib/firmware/raspberrypi/bootloader/default/"
      fi
