...
do_boot_order() {
  ...
  if [ $? -eq 0 ]; then
    ...
    if grep -q "stable" /etc/default/rpi-eeprom-update ; then
      EEPATH="${EEBASE}/stable/pieeprom*.bin"
    else
      EEPATH="${EEBASE}/critical/pieeprom*.bin"
    fi
...
do_boot_rom() {
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ "$DEFAULTS" -eq 0 ]; then # yes
      EEBASE=$(rpi-eeprom-update | grep RELEASE | sed 's/.*(//g' | sed 's/[^\/]*)//g')
      if grep -q "stable" /etc/default/rpi-eeprom-update ; then
        EEPATH="${EEBASE}/stable/"
      else
        EEPATH="${EEBASE}/critical/"
      fi
...
do_interface_menu() {
  ...
  if [ $RET -eq 1 ]; then
    ...
    case "$FUN" in
      ...
      I3\ *) do_spi ;;
      I4\ *) do_i2c ;;
      I5\ *) if is_pifive ; then do_serial_pi5 ; else do_serial ; fi ;;
      I6\ *) do_onewire ;;
      I7\ *) do_rgpio ;;
      *) whiptail --msgbox "Programmer error: unrecognized option" 20 60 1 ;;
    esac || whiptail --msgbox "There was an error running option $FUN" 20 60 1
...
do_advanced_menu() {
  if gpu_has_mmu ; then
    FUN=$(whiptail --title "Raspberry Pi Software Configuration Tool (raspi-config)" --menu "Advanced Options" $WT_HEIGHT $WT_WIDTH $WT_MENU_HEIGHT --cancel-button Back --ok-button Select \
      ...
      "A4 Boot Order" "Choose network or USB device boot" \
